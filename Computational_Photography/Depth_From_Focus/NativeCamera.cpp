...
void NativeCamera::draw(void) {
	...
	if (m_takeShot) {
		LOGI("taking shot");
		//TODO:
		//Set the focusDistance for different shots.
		//Hint: use m_request.settings.lens.focusDistance to set the settings.

		m_request.settings.lens.focusDistance +=
				(m_staticProperties.lens.minimumFocusDistance
						- m_staticProperties.lens.hyperFocalDistance) / 45;
