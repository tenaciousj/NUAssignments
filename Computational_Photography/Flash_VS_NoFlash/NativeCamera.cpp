...
void NativeCamera::startCamera() {
	...
	status = m_cameraManager->queryStaticProperties(1, m_staticProperties);
	//TODO
	//Change it to the front camera.
...
void NativeCamera::capture() {
	...
	//I think i need to add this for jpg
...
void NativeCamera::draw(void) {
	...
	if (m_takeShot) {
		...
		// From previous homeworks, you should know how to write images to disk. Instead of writeRAW function, use writeJPG function.
		// I deleted the code below -- Its easy to figure out.

		if (shotNum % 2 == 0 && shotNum < 11) {
			fName = fName + "White";
